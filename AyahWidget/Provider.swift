...
struct Provider: AppIntentTimelineProvider {
    func placeholder(in context: Context) -> VerseEntry {
        ...
        return entry(for: now(), mode: .both, interval: .daily, background: .liquidGlass, sizeClass: VerseSizeClass(family: context.family))
    }
    ...
    func snapshot(for configuration: AyahConfigIntent, in context: Context) async -> VerseEntry {
        ...
        return entry(for: now(), mode: configuration.displayMode, interval: configuration.refreshInterval, background: configuration.widgetBackground, sizeClass: VerseSizeClass(family: context.family))
    }

    func timeline(for configuration: AyahConfigIntent, in context: Context) async -> Timeline<VerseEntry> {
        let current = now()
        let sizeClass = VerseSizeClass(family: context.family)
        ...
        let entries = [
            entry(for: current, mode: configuration.displayMode, interval: configuration.refreshInterval, background: configuration.widgetBackground, sizeClass: sizeClass),
            entry(for: next, mode: configuration.displayMode, interval: configuration.refreshInterval, background: configuration.widgetBackground, sizeClass: sizeClass),
        ]
